...
<Warning>
The last point is subtle and got fixed during development. Pyth publishes roughly every 400ms,
so a 60-second window contains around 150 candidate updates. If any update in the window were
acceptable, a bettor could fetch them all and submit whichever one wins them the market.
Atrum uses the variant that pins the result to the **first** update at or after the target and
proves no earlier one exists.
</Warning>
