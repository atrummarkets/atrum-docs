...
| `marketId` | which market it belongs to — `0` until the note is bet; a deposit never names one |
...
|---|---|---|---|
| **unbet collateral** | yes | yes, as a 1:1 refund | **yes — directly, without betting** |
| **YES position** | no | if YES won | no |
| **NO position** | no | if NO won | no |
| **settled payout** | no | **no** | yes |

**Unbet collateral has its own direct exit.** Withdraw a note that was never bet and it comes
straight back to you — no redeem step, no market involved. The proof itself decides which of
the two withdrawal paths is legal: it derives a 1-bit `unbetExit` flag from whether the spent
note's `marketId` is zero, and publishes only that bit, never which market. That flag replaced
what used to be the full `marketId` in a withdrawal's public signal — which, for a winner
cashing out a settled payout, publicly named the market they'd won. Narrowing it to one bit
closed that leak.
